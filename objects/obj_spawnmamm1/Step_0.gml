if(spawnCount < maxSpawns)
{
	doSpawn = true
}
else
{
	doSpawn = false
}

if(canSpawn && doSpawn)
{ 
	var enemy = instance_create_layer(irandom_range
		(ev_boundary + (self.sprite_width/2), room_width - ev_boundary - (self.sprite_width / 2)),
		irandom_range(ev_boundary + (self.sprite_height/2), room_height - ev_boundary - (self.sprite_height / 2)), "PlayerLayer", obj_mammoth)
	enemy.spawner = self
	
	self.spawnCount += 1
	self.canSpawn = false
	
	alarm[0] = room_speed * spawnDelay + random_range(3, 6.0)
}
//self.x = irandom_range(ev_boundary + (self.sprite_width/2), room_width - ev_boundary - (self.sprite_width / 2));
//self.y = irandom_range(ev_boundary + (self.sprite_height/2), room_height - ev_boundary - (self.sprite_height / 2));
